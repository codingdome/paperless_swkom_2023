...
const DocumentBrowser = () => {
    ...
    const handleSearchClick = () => {
        ...
        if (!searchText || searchText.trim() === "") {
            ...
            setShowLoading(false);
            return; // Optionally handle empty search text case
        }

        fetch(`http://localhost:8081/find/by/${encodeURIComponent(searchText)}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                setFiles(data); // Update the files state with the search results
                setShowLoading(false); // Set loading to false after the search is complete
            })
            .catch(error => {
                console.error('Error during search:', error);
                toast.error(error);
                setShowLoading(false); // Ensure loading is set to false even if there's an error
    ...
    return (
        <div className="w-full h-full">
            ...
            <Container>
                ...
                <FilterNavbar files={files} setFiles={setFiles} sortOrder={sortOrder} setSortOrder={setSortOrder} searchText={searchText} setSearchText={setSearchText} handleSearchClick={handleSearchClick} showLoading={showLoading} setShowLoading={setShowLoading} />
